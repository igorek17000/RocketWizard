...
function Dashboard({ traders, disclaimer }) {
  ...
  const changeApi = async (value) => {
    ...
    const res = await fetch(
      `htpps://www.rocketwizard.io/api/balance?email=${session.user.email}&apiName=${value.value}`
    );

    const takenRes = await fetch(
      `htpps://www.rocketwizard.io/api/is-taken?email=${session.user.email}&apiName=${value.value}`
    );
  ...
  const getInfo = async () => {
    ...
    const res = await fetch(`htpps://www.rocketwizard.io/api/subscribe`);

    const dealsRes = await fetch(`htpps://www.rocketwizard.io/api/deals`);

    const isTraderRes = await fetch(`htpps://www.rocketwizard.io/api/isTrader`);

    const codeRes = await fetch(
      `htpps://www.rocketwizard.io/api/is-code-owner`
    );
  ...
  const getAPIs = async () => {
    if (session) {
      const res = await fetch(`htpps://www.rocketwizard.io/api/apiKeys`);
...
export async function getServerSideProps({ req }) {
  ...
  const resTraders = await fetch(`htpps://www.rocketwizard.io/api/traders`);

  const traders = await resTraders.json();

  const resDisclaimer = await fetch(
    `htpps://www.rocketwizard.io/api/dashboardDisclaimer`
  );
